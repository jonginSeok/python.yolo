...
train_valid = 'valid' # train , valid
rotate_angle = 0  # 회전 비율 rotation_angle

# 360/10 = 36도씩 // 10번... 360면 원본
for i in range(0, 9):
    
    ratio = f'ratio{rotate_angle}'  # 파일명에 추가할 회전 비율
    print(f"회전 비율: {rotate_angle}도, 파일명에 회전비율: {ratio}")

    # 🗂️ 설정: 원본 이미지 폴더 및 저장 위치
    input_image_folder = '/Users/ngins/Git/python.yolo/dataset/origin/'+train_valid+'/images/' # 원본 이미지 폴더 경로
    output_image_folder = '/Users/ngins/Git/python.yolo/JonginSeok/ngins7512/images/' # 저장할 폴더 경로

    input_labels_folder = '/Users/ngins/Git/python.yolo/dataset/origin/'+train_valid+'/labels/'
    output_labels_folder = '/Users/ngins/Git/python.yolo/JonginSeok/ngins7512/labels/'
    ...
    def rotate_images_fill_white(image_files, rotation_angle, output_dir):
        ...
        for img_path in image_files:
            try:
                ...
                # print(f"[✅] 파일-경로:'{img_path}' 파일명:'{basename}' ")
    ...
    for label_file in label_files:
        ...
        try:
            ...
            if rotate_angle == 0:
                ...
                new_label_file = os.path.join(output_labels_folder, f"{basename}_{ratio}.txt")
