...
class BottleDatasetWithSize(Dataset):
    def __init__(self, root_dir, label_map, transform=None):
        ...
        for class_name, label in label_map.items():  # {'bad-broken_large':0,'bad-broken_small':1,'bad-contamination':2,'bottle-good':3}
...
class BottleCNNWithSize(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Sequential(  # 3channel, 16filters, filter-size 3
            nn.Conv2d(3, 32, 3, padding=1),  # 224x224x3 -> 224x224x16, padding=1은 1픽셀 추가하여 출력크기 유지
            nn.ReLU(),
...
def predict_image(image_path):
    ...
    area = orig_w * orig_h
    aspect_ratio = orig_w / orig_h
    size_features = torch.tensor([orig_w, orig_h, area, aspect_ratio], dtype=torch.float32).to(DEVICE)
...
predict_image(data_path+'/test/bad-contamination/012_png.rf.649902ef1adcb13616394303e1fd0bdd_ratio80.jpg')  # 실제 파일 경로 지정