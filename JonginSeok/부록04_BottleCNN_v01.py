...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        self.fc = nn.Sequential(
            ...
            nn.Linear(128, 4)     # BCEWithLogitsLoss() (또는 BCELoss + Sigmoid),	CrossEntropyLoss() (Softmax 포함)
        )
...
for epoch in range(EPOCHS):
    ...
    with torch.no_grad():
        for x, y in valid_loader:
            x, y = x.to(DEVICE), y.to(DEVICE) # Tensor 데이터를 지정된 디바이스(CPU 또는 GPU)로 이동시키고, 새 참조 리턴
            outputs = model(x)
            correct += (outputs.argmax(1) == y).sum().item()
            total += y.size(0)
